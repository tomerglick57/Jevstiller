...
```bash
docker run -d --name jevstiller -p 8080:8080 -v jevstiller-data:/data ghcr.io/tomerglick57/jevstiller:0.3.4
...
- It works with a read-only root filesystem when `/tmp` is writable, and with all capabilities dropped. `deploy/smoke_test.py` checks this, and more, against any image: `python deploy/smoke_test.py ghcr.io/tomerglick57/jevstiller:0.3.4`. CI runs it on every image build.
...
| Disk | Per task, bounded. The store keeps at most ~80,000 rows (`max_train_samples` + `max_calib_samples` + `keep_local_rows`) of ~3–4 KB each (text, embedding, answers and indexes): ~300 MB for a busy task, less for a quiet one. Add ~8 MB per kept model version (production, shadow and at most `keep_versions`, 3, of each finished state). `idle_ttl_days` deletes tasks nobody uses. |
